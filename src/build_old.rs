...
fn main() {
    ...
    for theme in sitecfgthemes.keys() {
        ...
        std::fs::write(format!("./static/{}.css", &theme), &header).unwrap_or_else(|_| panic!("Unable to write file {}.css", &theme));
    }

    // Step 1.1: Build and move print CSS
    let printscss = read_file("src/styling/print.scss").unwrap();
    let printcss = grass::from_string(printscss, &grass_options).unwrap();
    std::fs::write("./static/print.css", printcss).expect("Unable to write file print.css");
